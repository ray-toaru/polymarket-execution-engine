...
- The armed CLI writes the report file at every remote-side-effect stage. If post status is unknown, post is accepted, cancel status is unknown, or cancel confirmation fails, the report file must contain a structured `operator_required` or stage report rather than relying on terminal output. Each stage is also appended to `<report-file>.stages.jsonl`, while `<report-file>` keeps the latest stage or final receipt for operator handoff.
