...
def validate_v15_admin_audit_and_runtime_provider(spec: dict | None = None) -> None:
    ...
    require_tokens(
        ...
        [
            'std::env::var("PMX_TEST_DATABASE_URL")',
            'std::env::set_var("PMX_API_SERVICE_TOKEN", "service-token-pg-audit")',
            'std::env::set_var("PMX_API_ADMIN_TOKEN", "admin-token-pg-audit")',
            "pmx_api::try_postgres_app(database_url.clone(), true)",
            '"/v1/admin/kill-switch"',
            "StatusCode::ACCEPTED",
            '"/v1/admin/cancel-order"',
...
def validate_v23_lifecycle_query_and_hardening(spec: dict | None = None) -> None:
    ...
    store_postgres_admin_audit = (STORE_SRC / "postgres_audit/admin.rs").read_text()
    ...
    require_tokens(
        postgres_admin_audit_list_body,
        "current postgres admin audit list path",
        [
